...
gulp.task 'site:js:cat', ->
  jsOrder   = ['angular.min.js','lodash.min.js']
...
gulp.task 'site:js:hash', ->
  hasher.writeHash('./dest/_temp/all.cjs', (md5) ->
    hash = "all-#{md5}.js"
    vals.js = hash: '/js/' + hash
    path.join(process.cwd(), './dest/_site/js', hash))


gulp.task 'site:css:hash', ->
  hasher.writeHash('./dest/_temp/all.combined.css', (md5) ->
    hash = "combined-#{md5}.css"
    vals.css = hash: '/css/' + hash
    path.join(process.cwd(), './dest/_site/css', hash))


gulp.task 'font:hash', ->
  siteFile = path.join("./dest/_temp/", "fonts.css")
  fontHasher(model.fonts, siteFile, "./dest/_temp", "./dest/_site/font")


gulp.task 'dir:_site', (cb) -> run('mkdir -p dest/_site dest/_site/{js,css,html,font}').exec(cb)
gulp.task 'site', seq(
...
gulp.task 'site:assets', ->
  buildAssets = ['dest/_temp/*.html']
  gulp.src(buildAssets).pipe(dest('./dest/_site/html'))
